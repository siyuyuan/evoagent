...
class Multi_Planner:
    ...
    def run(self, text, query, description, log_file=None) -> str:
        if log_file:
            log_file.write('\n---------------Planner\n'+self._build_agent_prompt(text, query, description))
        # print(self._build_agent_prompt(text, query, description))
        if self.model_name in ['gemini']:
            return str(self.llm.invoke(self._build_agent_prompt(text, query, description)).content)
        else:
            if len(self.enc.encode(self._build_agent_prompt(text, query, description))) > 12000:
                return 'Max Token Length Exceeded.'
            else:
                while True:
                    try:
                        answer = self.llm([HumanMessage(content=self._build_agent_prompt(text, query, description))]).content
                        return answer
                    except:
                        continue

    def _build_agent_prompt(self, text, query, description) -> str:
        return self.agent_prompt.format(
            text=text,
            query=query,
            description=description
...
class Self_Refine_Planner:
    ...
    def run(self, text, query, answer, feedback, log_file=None) -> str:
        if log_file:
            log_file.write('\n---------------Planner\n'+self._build_agent_prompt(text, query, answer, feedback))
        # print(self._build_agent_prompt(text, query, answer, feedback))
        if self.model_name in ['gemini']:
            
            return str(self.llm.invoke(self._build_agent_prompt(text, query, answer, feedback)).content)
        else:
            if len(self.enc.encode(self._build_agent_prompt(text, query, answer, feedback))) > 12000:
                return 'Max Token Length Exceeded.'
            else:
                while True:
                    try:
                        ...
                        #pdb.set_trace()
                        #print(self._build_agent_prompt(text, query, answer, feedback))
                        answer = self.llm([HumanMessage(content=self._build_agent_prompt(text, query, answer, feedback))]).content
                        return answer
                    except:
                        continue

    def _build_agent_prompt(self, text, query, answer, feedback) -> str:
        return self.agent_prompt.format(
            text=text,
            query=query,
            answer=answer,
